...
| `resolve-bug-issues` | Clears a bug backlog in one supervised batch: lists open `bug`-labelled issues, fans out one parallel subagent per issue (each in its own isolated worktree) to reproduce the bug with a failing test, ship a minimal fix on a `fix/<n>-<slug>` branch, and open a PR with `Closes #n`. Then coordinates — reviews each PR, merges the green ones in `Blocked by` dependency order, verifies issues closed, and reports an issue → branch → PR → status table. Never commits to main. Slash-invoke only. | Burning down a batch of bug tickets in parallel, branch-per-issue and PR-per-issue, without hand-driving each one sequentially. |
