...
| --- | --- | --- |
| `brainstorming` | Turns ideas into approved designs before any code is written. Classifies each request as spike / bounded / architectural, asks clarifying questions one at a time, presents a design scaled to the task, and hard-gates implementation on explicit approval — user-visible UI changes get a faithful browser preview (real theme, working behavior, no annotations) before the approval gate, and approved bounded designs get one more question: implement directly, or publish issues via `/gt-engineering:to-issues`? Architectural work gets a committed spec doc (`docs/specs/`) and hands off to `/gt-engineering:to-issues` (optionally via `/to-product-req`); the browser-based visual companion (self-contained Node server, no deps, session files in `.gt/brainstorm/`) shows mockups and diagrams when a question is clearer shown than told. | Any creative work — new features, components, or behavior changes — where you want intent, requirements, and design nailed down and approved before implementation starts. |
